...
async def exception_handler(request, exc: Exception) -> JSONResponse: return JSONResponse(status_code=getattr(exc, 'http_status_code', 500), content=mkresp('error', type(exc).__name__, str(exc)))
