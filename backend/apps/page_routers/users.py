...
async def accesses(request: Request, username: str):
    ...
    return await render('pages/users/accesses', {'user': user, 'target_user': {'username': target_user.username, 'name': target_user.name}, 'accesses': {access: await target_user.has_access(access, allow_admin=False) for access in (await Access.get_all()).values()}}, expose='target_user')