...
def log(name: str, message: str, level: str = default_level, level_color: 'str | None' = None) -> str: return f'{Style.RESET_ALL + Style.DIM + datetime.now().strftime(strftime) + Style.RESET_ALL} {Fore.CYAN + Style.BRIGHT + pid + Style.RESET_ALL} {Fore.BLUE + Style.BRIGHT + name + Style.RESET_ALL}: [{Style.BRIGHT + (level_colors[level_color.upper() if level_color != None and level_color.upper() in level_colors else level.upper() if level.upper() in level_colors else default_level]) + level + Style.RESET_ALL}] {((message[0].upper() + message[1:]) if len(message) > 0 else message) + (suffix if not message.endswith(suffix) else nothing)}'